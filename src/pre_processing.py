...
DATA_SET_PATH = '/home/steffen/Projects/FH/seminar/datasets'
...
def create_entry(genres, x_spect, data):
    (index, row) = data
    try:
...
def create_array(df):
    ...
    func = partial(create_entry, genres, x_spect)

    for data in tqdm.tqdm(pool.imap_unordered(func, df.iterrows()), total=df.shape[0], ncols=100 ):
        try:
            spect, genre = data
            x_spect = np.append(x_spect, [spect], axis=0)
            genres.append(genre)
        except:
            print("Couldn't process: ", x_spect.shape[0] + 1)
